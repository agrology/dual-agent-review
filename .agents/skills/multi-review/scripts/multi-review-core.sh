#!/usr/bin/env bash
# multi-review-core.sh — deterministic marker/thread logic for multi-review review.
...
warn_missing_disclosure() { # <doc> — non-blocking: warn if a marker line lacks a "> —" follow-up
  ...
  ')"
  (( missing > 0 )) && echo "multi-review-core: warning: ${missing} comment(s) missing a '> — via <model>' disclosure line" >&2
  return 0
}
...
write_marker() { # <doc> <new-marker-line> — atomic replace of the single marker line
  ...
  if awk -v new="$new" '
    /<!--[[:space:]]*multi-review:/ && !done { print new; done=1; next }
    { print }
...
cmd_next_marker() { # <doc> <event>
  ...
  new_marker="<!-- multi-review: ${new_state} · round ${new_round}/${MK_MAX} -->"
