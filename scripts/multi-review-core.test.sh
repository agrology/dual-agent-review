#!/usr/bin/env bash
# multi-review-core.test.sh — deterministic marker/thread logic.
set -uo pipefail
...
grep -qF '+<!-- multi-review: converged · round 3/10 -->' "$D" && ok "embedded diff marker left untouched" || bad "next-marker mangled the embedded diff marker"
...
D="$(mkdoc nm1.md '<!-- multi-review: awaiting-author · round 2/10 -->')"
...
D="$(mkdoc nm2.md '<!-- multi-review: awaiting-author · round 10/10 -->')"
bash "$SUT" next-marker "$D" author-done >/dev/null 2>&1
out="$(bash "$SUT" marker "$D" 2>/dev/null)"
...
D="$(mkdoc nm3.md '<!-- multi-review: awaiting-reviewer · round 1/10 -->')"
before="$(cat "$D")"
...
<!-- multi-review: converged · round 4/10 -->
...
<!-- multi-review: converged · round 3/10 -->
...
D="$(mkdoc perms.md '<!-- multi-review: awaiting-author · round 2/10 -->')"
chmod 644 "$D"
bash "$SUT" next-marker "$D" author-done >/dev/null 2>&1
...
<!-- multi-review: converged · round 2/10 -->
