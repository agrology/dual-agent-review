#!/usr/bin/env bash
# dual-agent-watch.sh <doc> <token> — block until <doc> changes, then exit 0.
...
# polling mtime is content-accurate everywhere. Latency <= DUAL_AGENT_WATCH_INTERVAL,
# which is negligible for a human-paced review loop.
set -uo pipefail

doc="${1:?usage: dual-agent-watch.sh <doc> <token>}"
token="${2:?usage: dual-agent-watch.sh <doc> <token>}"
: "$token"   # carried for the lock logic added in Task 8
[[ -e "$doc" ]] || { echo "dual-agent-watch: doc not found: $doc" >&2; exit 2; }
...
docdir="$(cd "$(dirname "$doc")" 2>/dev/null && pwd -P)" \
  || { echo "dual-agent-watch: cannot resolve doc dir: $doc" >&2; exit 2; }
doc="${docdir}/$(basename "$doc")"

lockdir="${doc}.dual-agent-watch.lock"
...
is_matching_watcher() { # <pid> <recorded-token>
  ...
  [[ "$cmd" == *dual-agent-watch.sh* && "$cmd" == *"$(basename "$doc")"* && "$cmd" == *"$tok"* ]]
}
...
acquire_lock() {
  ...
  while ! mkdir "$lockdir" 2>/dev/null; do
    ...
    (( ++tries > 100 )) && { echo "dual-agent-watch: cannot acquire lock: $lockdir" >&2; exit 3; }
  done
  printf '%s\n' "$$"     > "${lockdir}/pid"
  printf '%s\n' "$doc"   > "${lockdir}/doc"
  printf '%s\n' "$token" > "${lockdir}/token"
...
interval="${DUAL_AGENT_WATCH_INTERVAL:-2}"
start="$(sig "$doc")"
