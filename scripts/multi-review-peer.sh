#!/usr/bin/env bash
# multi-review-peer.sh — symmetric peer-review grammar for PR mode. The coordination transport
# (marker, watcher, wait, next-marker) stays in multi-review-core.sh; this owns only the
...
die() { echo "multi-review-peer: $1" >&2; exit "${2:-1}"; }

MODE_GREP='<!--[[:space:]]*multi-review-mode:'
...
MODE_RE='^[[:space:]]*<!--[[:space:]]*multi-review-mode:[[:space:]]*([a-z-]+)[[:space:]]*-->[[:space:]]*$'
...
_table() {
  ...
  review_section "$doc" | strip_fences /dev/stdin | awk '
    function fail(m){ print "multi-review-peer: " m > "/dev/stderr"; exit 2 }
...
cmd_compose_review() { # <doc> -> joint-review body on stdout
  ...
  printf '%s\n' "$t" | awk -F'\t' '
    ...
    END {
      ...
      if (models == "") printf "———\n🤖 Posted by AI agents via multi-review peer review.\n"
      else printf "———\n🤖 Posted by AI agents (%s) via multi-review peer review.\n", models
    }
  '
}
...
cmd_compose_inline() { # <doc> -> "path\tstart\tend\tbody" per agreed+anchored finding
  ...
  printf '%s\n' "$t" | awk -F'\t' '
    ...
    $3 == "agreed" && $7 != "" {
      ...
      emoji = (sev=="high") ? "🔴" : (sev=="med") ? "🟠" : (sev=="low") ? "🟡" : ""
      body = emoji " " sev " — " concern " — risk: " risk " — 🤖 multi-review peer review (" raiser
...
cmd_mode() { # <doc> -> peer-review | asymmetric
  ...
  (( n == 1 )) || die "multiple multi-review-mode hints in header: $doc" 1
  line="$(printf '%s\n' "$hdr" | grep -E "$MODE_GREP" | head -1)"
  [[ "$line" =~ $MODE_RE ]] || die "malformed multi-review-mode hint: $doc" 1
  state="${BASH_REMATCH[1]}"
  case "$state" in
    peer-review) echo "peer-review" ;;
    *) die "unknown multi-review-mode '${state}': $doc" 1 ;;
  esac
}
