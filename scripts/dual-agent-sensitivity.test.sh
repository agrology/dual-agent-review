#!/usr/bin/env bash
# dual-agent-sensitivity.test.sh — no company-sensitive terms in the SHIPPING tree.
# Terms come from a single non-shipping source so no scanned file embeds them.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "${DIR}/.." && pwd)"
TERMS_FILE="${DUAL_AGENT_TERMS_FILE:-${ROOT}/.dual-agent/sensitivity-terms.txt}"
fails=0
ok()  { echo "  ok: $1"; }
...
HITS="$(cd "$ROOT" && grep -rIinE "$TERMS" \
  ...
  --exclude-dir=superpowers --exclude-dir=.superpowers --exclude-dir=.dual-agent \
  . 2>/dev/null)"
