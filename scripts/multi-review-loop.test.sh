#!/usr/bin/env bash
# multi-review-loop.test.sh — one full deterministic round over the core helper,
# with stubbed reviewer/author edits. No LLM, no network, no watcher.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CORE="${DIR}/multi-review-core.sh"
GUARD="${DIR}/multi-review-egress-guard.sh"
fails=0
...
# guard accepts it
MULTI_REVIEW_DOC_DIRS="$SPECS" bash "$GUARD" "$DOC" >/dev/null 2>&1 && ok "guard accepts the spec" || bad "guard rejected the spec"
