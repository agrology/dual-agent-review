#!/usr/bin/env bash
# multi-review-history-check.sh — scan ALL git history for company-sensitive terms.
# Terms come from .multi-review/sensitivity-terms.txt (single source; resolved relative to this
# script), overridable via MULTI_REVIEW_TERMS_FILE. Absent term file => clean pass (public clone).
# Usage: multi-review-history-check.sh [repo-dir]   (default: cwd)
# Exit 0 = clean/none-configured; 1 = matches found (printed); 2 = usage/not-a-repo.
set -uo pipefail
SELF="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TERMS_FILE="${MULTI_REVIEW_TERMS_FILE:-${SELF}/../.multi-review/sensitivity-terms.txt}"
repo="${1:-.}"
