#!/usr/bin/env bash
# multi-review-history-check.test.sh — the history scanner flags sensitive strings in ANY commit.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUT="${DIR}/multi-review-history-check.sh"
fails=0
ok()  { echo "  ok: $1"; }
...
export MULTI_REVIEW_TERMS_FILE="$TERMS_TMP"
cleanup() {
  rm -f "$TERMS_TMP"
...
if MULTI_REVIEW_TERMS_FILE=/nonexistent/terms.txt bash "$SUT" "$R" >/dev/null 2>&1; then ok "absent term file => clean pass"; else bad "absent term file should exit 0"; fi
