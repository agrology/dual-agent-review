#!/usr/bin/env bash
# dual-agent-egress-guard.test.sh — path/egress validation, multi-dir.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUT="${DIR}/dual-agent-egress-guard.sh"
fails=0
...
SCRATCH="${WORK}/.dual-agent/reviews/o/r"
mkdir -p "$SCRATCH"
echo "# pr review" > "${SCRATCH}/pr-1.md"

DD="${SPECS} ${PLANS}"
g() { DUAL_AGENT_DOC_DIRS="$DD" bash "$SUT" "$1" >/dev/null 2>&1; echo $?; }
...
gw() { ( cd "$WORK" && DUAL_AGENT_DOC_DIRS="$DD" bash "$SUT" "$1" >/dev/null 2>&1; echo $? ); }
got="$(gw '.dual-agent/reviews/o/r/pr-1.md')"
[[ "$got" == "0" ]] && echo "  ok: accepts a scratch file under .dual-agent/reviews" \
