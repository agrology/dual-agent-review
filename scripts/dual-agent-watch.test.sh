#!/usr/bin/env bash
# dual-agent-watch.test.sh — mtime watcher (poll-only) + lock.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUT="${DIR}/dual-agent-watch.sh"
fails=0
WORK="$(mktemp -d)"; trap 'rm -rf "$WORK"' EXIT
export DUAL_AGENT_WATCH_INTERVAL=1
...
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOC" tok-a & wpid=$!
sleep 1
printf 'x\n' >> "$DOC"            # change mtime
...
LOCKF="${DOCF}.dual-agent-watch.lock"
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOCF" tok-f & wpid=$!
...
echo "# e" > "${WORK}/e.md"
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "${WORK}/e.md" tok-b & wpid=$!
sleep 3
if ! is_dead "$wpid"; then
...
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOC3" tok-c & wpid=$!
sleep 1; rm -f "$DOC3"
sleep 2
if ! is_dead "$wpid"; then
...
LOCK="${DOC2}.dual-agent-watch.lock"
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOC2" tok-1 & w1=$!
sleep 2
[[ -d "$LOCK" ]] && echo "  ok: lockdir created" || { echo "  FAIL: no lockdir"; fails=$((fails+1)); }
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOC2" tok-2 & w2=$!
...
( cd "$WORK" && DUAL_AGENT_WATCH_INTERVAL=1 exec bash "$SUT" "relabs.md" tok-r1 ) & wr1=$!
sleep 2
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOCR" tok-r2 & wr2=$!
...
for _ in 1 2 3 4 5; do [[ "$(cat "${DOCR}.dual-agent-watch.lock/token" 2>/dev/null)" == "tok-r2" ]] && break; sleep 1; done
[[ "$(cat "${DOCR}.dual-agent-watch.lock/token" 2>/dev/null)" == "tok-r2" ]] && echo "  ok: one canonical lockdir taken over" || { echo "  FAIL: relative/absolute arming did not share a lockdir"; fails=$((fails+1)); }
printf 'x\n' >> "$DOCR"; sleep 2; kill "$wr2" 2>/dev/null

# --- lock: PID-reuse victim is NOT killed ---
DOC3="${WORK}/reuse.md"; echo "# r" > "$DOC3"
LOCK3="${DOC3}.dual-agent-watch.lock"
...
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOC3" tok-3 & w3=$!
sleep 2
...
LOCKL="${DOCL}.dual-agent-watch.lock"
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOCL" tok-l & wl=$!
sleep 2
...
LOCKI="${DOCI}.dual-agent-watch.lock"
mkdir -p "$LOCKI"             # created but never populated (simulates a populator that died)
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOCI" tok-i & wi=$!
...
LOCK4="${DOC4}.dual-agent-watch.lock"
DUAL_AGENT_WATCH_INTERVAL=1 bash "$SUT" "$DOC4" tok-4 & w4=$!
sleep 2; printf 'x\n' >> "$DOC4"
