#!/usr/bin/env bash
# multi-review-auto-step.sh — per-round verdict for the autonomous multi-review loop.
...
PEER="${DIR}/multi-review-peer.sh"

die()  { echo "multi-review-auto-step: $1" >&2; exit 2; }
stop() { echo "stop $1"; exit 20; }

doc="${1:-}"; prev_state="${2:-}"; prev_round="${3:-}"
[[ -n "$doc" && -n "$prev_state" && -n "$prev_round" ]] \
  || die "usage: multi-review-auto-step.sh <doc> <prev-state> <prev-round>"
