#!/usr/bin/env bash
# dual-agent-peer.test.sh — symmetric peer-review grammar.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUT="${DIR}/dual-agent-peer.sh"
fails=0
...
D="$(mkpeerdoc peer.md '<!-- dual-agent-mode: peer-review -->')"
...
D="${WORK}/bodyhint.md"
{ echo "# Doc"; echo; echo "## PR description"; echo '<!-- dual-agent-mode: peer-review -->'; } > "$D"
...
D="$(mkpeerdoc dup.md '<!-- dual-agent-mode: peer-review -->' '<!-- dual-agent-mode: peer-review -->')"
...
D="$(mkpeerdoc bad.md '<!-- dual-agent-mode: bogus -->')"
...
D="$(mkpeerdoc trail.md '<!-- dual-agent-mode: peer-review --> trailing')"
...
D="$(mkpeerdoc extra.md '<!-- dual-agent-mode: peer-review extra -->')"
...
{ echo "# PR review: X"; echo; echo '<!-- dual-agent-mode: peer-review -->'; echo "- **PR:** https://github.com/o/r/pull/1"; \
...
# Regression for the silent false-convergence bug (shared with dual-agent-core.sh): an open-but-
...
<!-- dual-agent-review: converged · round 1/10 -->
...
printf '%s\n' "$out" | grep -qF '🔴 high — agreed and anchored — risk: cache miss — 🤖 dual-agent peer review (gpt-5-codex + claude-opus-4-8)' && ok "compose-inline: body + disclosure" || bad "compose-inline body (got: $out)"
