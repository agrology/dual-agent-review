#!/usr/bin/env bash
# dual-agent-pr.test.sh — PR-mode ingest/publish logic (gh stubbed; no network).
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUT="${DIR}/dual-agent-pr.sh"
...
[[ "$out" == ".dual-agent/reviews/octocat/hello-world/pr-42.md" ]] && ok "scratch-path: identity-keyed" || bad "scratch-path (got '$out')"
...
grep -qxF '<!-- dual-agent-mode: peer-review -->' "$OUT" && ok "seed: writes mode hint" || bad "seed mode hint missing"
awk '/^## /{exit} {print}' "$OUT" | grep -qF 'dual-agent-mode: peer-review' && ok "seed: mode hint is in the header region" || bad "seed mode hint not in header"
...
[[ $code == 0 && "$path" == ".dual-agent/reviews/o/r/pr-8.md" ]] && ok "ingest: prints the scratch path" || bad "ingest path (got '$path' code $code)"
[[ -f "${WORK}/.dual-agent/reviews/o/r/pr-8.md" ]] && ok "ingest: creates the scratch file" || bad "ingest file missing"
grep -q '# PR review: My Title' "${WORK}/.dual-agent/reviews/o/r/pr-8.md" && ok "ingest: title from gh" || bad "ingest title missing"
grep -qF '**Author:** bob'       "${WORK}/.dual-agent/reviews/o/r/pr-8.md" && ok "ingest: author from gh" || bad "ingest author missing"
grep -qF '+added line'          "${WORK}/.dual-agent/reviews/o/r/pr-8.md" && ok "ingest: diff embedded" || bad "ingest diff missing"
